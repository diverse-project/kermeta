	...
	public void paint(Graphics2D g) {
		if(view.isVisible() && name!=null && card!=null && (name.length()>0 || card.length()>0)) {
			...
			if(view.entitySrc==view.entityTar) {
				Rectangle2D rec1 = new TextLayout(name, view.entitySrc.font, EntityView.FONT_RENDER_CONT).getBounds();
				Rectangle2D rec2 = new TextLayout(card, view.entitySrc.font, EntityView.FONT_RENDER_CONT).getBounds();
