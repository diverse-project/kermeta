    ...
    if (!Util.hasCompilerIgnoreTag(par)){
      ...
      if (!par.isIsAbstract()){
        if (Util.hasEcoreTag(par)){
          ...
          factoryDefClass append Util.protectScalaKeyword(" def create"+par.getName()+ param.toString +" : "+ Util.protectScalaKeyword(kermeta.utils.TypeEquivalence.getTypeEquivalence(genpackageName.toString+par.getName())+param.toString)+" = { new "+ packageName.toString+".Rich"+par.getName + param.toString +" }\n")
        }
        else{
          factoryDefClass append Util.protectScalaKeyword(" def create"+par.getName()+ param.toString +" : "+ Util.protectScalaKeyword(kermeta.utils.TypeEquivalence.getTypeEquivalence(packageName.toString+"."+par.getName())+param.toString)+" = { new "+ packageName.toString+".Rich"+par.getName+ param.toString +" }\n")
        }
      }
    }
  }
