...
trait EClassAspect extends implicitAspect with EClassKermetaMapping {
	//this:EClass => 
	def testGetReflection {
		//this.getClass().getFields().map(e => { println("scalaMeta"+e.getName()) })
		this.eClass().getEAllAttributes().filter{e=>  (!e.isDerived()) }.map{e => println("get on "+e.getName()+" : "+this.eGet(e))}  
	}
