...
public class KermetaUtils {
	...
	public KermetaUnit loadStdLib() { // KermetaUnit.STD_LIB_URI =
		...
		KermetaUnit unit = KermetaUnitFactory.getDefaultLoader()
				.createKermetaUnit(KermetaUnit.STD_LIB_URI);
		unit.load();
		return unit;
	}
	...
	/**
	 * Return true if the given type is a base type, false otherwise Basetypes
	 * are the one as defined in fr.irisa.triskell.kermeta.interpreter project,
	 * that are linked to real java types, that is :
	 *  - Boolean - Character - Collection - Integer - Iterator - Map - Numeric
	 * (Real?) - String
	 */
	public boolean isBaseType(Type type) {
		Boolean isbasetype = false;
		if (type instanceof Class) {
			Class fclass = (Class) type;

			String classdef = fclass.getTypeDefinition().getName();
			// Having the below types in another way then manual
			String[] basetypes = new String[] { "Boolean", "Character",
					"Integer", "Iterator", "Map", "Numeric", "String" };
			String[] colltypes = new String[] { "Collection", "Set",
					"OrderedSet", "Sequence", "OrderedSequence", "Bag" };
	...
	public boolean isStandardType(Type type) {
		...
		if (typedef_qname != null) {
			isstandardtype = standardUnit
					.getTypeDefinitionByName(typedef_qname) != null
	...
	/*
	 * Special methods used to handle some mappings between the Type concept and
	 * the TypeDefinitions concept. We have to think about which place would be
	 * most relevant?
	 * 
	 */
	public void addSuperTypeToClassDefinition(ClassDefinition classdef,
			TypeDefinition supertypedef) {
		...
		if (toadd == null) // Already controlled by the check methods in the
							// policies
		{
			classdef.getContainedType().clear();
			classdef.getSuperType().add(
					createTypeForTypeDefinition(supertypedef));
	 ...
	 * @param supertypedef
	 * @param classdef
	 * @return a Type element if type related to supertypedef is in the
	 *         supertypes of classdef, null otherwise
	 */
	public Type findSuperTypeInClassDefinition(ClassDefinition classdef,
			TypeDefinition supertypedef) {
		Type result = null;
		...
		while (it.hasNext() && result == null) {
			Type type = it.next();
			// Class is the only type linked to a ClassDefinition, and we are
			// looking for a type corresponding
			// to a ClassDefinition!
			if (type instanceof Class) {
				// System.err.println("source:" + classdef.getName() +
				// supertypedefname);
				if (standardUnit.getQualifiedName(
						((Class) type).getTypeDefinition()).equals(
						supertypedefname))
					result = type;
			}
		}
	...
	/**
	 * We have to manage this operation using qualified names, there is no way
	 * to retrieve types to remove..
	 * 
	 * @param supertype
	 * @param typedef
	 */
	public void removeSuperTypeFromClassDefinition(ClassDefinition classdef,
			TypeDefinition supertypedef) {
	...
	/**
	 * Equivalent to : classdef.contains(the_type_related_to_supertypedef) Used
	 * because graphical elements between which we are testing the inheritance
	 * relation are directly linked to ClassDefinition model elements.
	 */
	public boolean isContainedSuperTypeInClassDefinition(
			ClassDefinition classdef, TypeDefinition supertypedef) {
		return findSuperTypeInClassDefinition(classdef, supertypedef) != null;
	}

	/**
	 * If a property of type targetdef already exists in the given srcdef,
	 ...
	 */
	public boolean existsPropertyBetweenClassDefinitions(
			ClassDefinition srcdef, ClassDefinition targetdef) {
		return false;
	}
