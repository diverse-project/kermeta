...
public class CallFeatureDebugInterpreter {
	

	static private void stopIfNecessary(CallFeature node, DebugInterpreter interpreter, boolean left) {
		// Take care about the state of interpreter. Calling stopIfNecessary when the interpreter is terminated leads to thread blocking.
		if ( ! interpreter.isTerminated() ) {
			...
			if ( hasStopped ) {
				...
				case STEPPING_INTO :
					if ( left ) {
						CallFeature leftCall = DebuggerHelper.getLeftCallableExpression(node);
						interpreter.stepInto(leftCall);
					} else
						interpreter.stepInto(node);
					break;
	...
	static public RuntimeObject doIt(CallFeature node, DebugInterpreter interpreter) {
		
		stopIfNecessary(node, interpreter, true);
...
//			 Get the parameters of this operation
			List<RuntimeObject> parameters = new ArrayList<RuntimeObject>();
			for ( Expression e : node.getParameters() ) {
				parameters.add( (RuntimeObject) interpreter.accept( e ) );
			}
			...
			try {
				if(isJarProxy){
					...
					stopIfNecessary(node, interpreter, false);
