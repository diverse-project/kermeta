...
//	protected Hashtable allClasses;
	...
	public Object visit(FBlock node) {
		...
		FClassDefinition blockClassDef=(FClassDefinition)nodeMetaclass.getData().get("FClassDefinition");
		RuntimeObject knode=nodeMetaclass.getFactory().createClassFromClassDefinition(nodeMetaclass);
		Hashtable data=new Hashtable();
		data.put("kcoreObject",node);
		knode.setData(data);
	...
	public Object visit(FRescue node) {
		...
		FClassDefinition rescueClassDef=(FClassDefinition)nodeMetaclass.getData().get("FClassDefinition");
		RuntimeObject knode=nodeMetaclass.getFactory().createClassFromClassDefinition(nodeMetaclass);
		Hashtable data=new Hashtable();
		data.put("kcoreObject",node);
		knode.setData(data);
	...
	public Object visit(FClass node) {
		...
		if (node.getFTypeParamBinding().size() > 0) {
			FClassDefinition classClassDef=(FClassDefinition)nodeMetaclass.getData().get("FClassDefinition");
	...
	public Object visit(FEnumeration node) {
		...
		FClassDefinition enumerationClassDef=(FClassDefinition)nodeMetaclass.getData().get("FClassDefinition");
		knode=nodeMetaclass.getFactory().createClassFromClassDefinition(nodeMetaclass);
		Hashtable data=new Hashtable();
		data.put("kcoreObject",node);
		knode.setData(data);
		knode.getProperties().put("name",node.getFName());
	...
	public Object visit(FLambdaExpression node) {
		...
		if (node.getFParameters().size()> 0) {
			RuntimeObject exprCallMetaclass=(RuntimeObject)Run.koFactory.getTypeDefinitionByName("kermeta::language::behavior::CallExpression");
			FClassDefinition callExprClassDef=(FClassDefinition)exprCallMetaclass.getData().get("FClassDefinition");
			FProperty parameters=findProperty(callExprClassDef,"parameters");
			RuntimeObject parametersNode=ReflectiveCollection.createReflectiveCollection(knode,createPropertyNode(parameters));
			knode.getProperties().put("parameters",parametersNode);
			this.currentParamList=parametersNode;
...
/*		String result = "package " + ppIdentifier(node.getFName()) + "\n";
	...
	public Object visit(FProductType node) {
		...
		FClassDefinition productClassDef=(FClassDefinition)productMetaclass.getData().get("FClassDefinition");
...
/*		if (node.getFOpposite() != null)
    ...
    public Object visit(FTag node) {
		RuntimeObject nodeMetaclass=(RuntimeObject)Run.koFactory.getTypeDefinitionByName("kermeta::language::behavior::Tag");
		RuntimeObject knode=nodeMetaclass.getFactory().createClassFromClassDefinition(nodeMetaclass);
		Hashtable data=new Hashtable();
		data.put("kcoreObject",node);
		knode.setData(data);
