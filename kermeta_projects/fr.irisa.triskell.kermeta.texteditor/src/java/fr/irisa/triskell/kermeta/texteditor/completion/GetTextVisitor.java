...
public class GetTextVisitor extends KermetaOptimizedVisitor {
	...
	public Object visitClassDefinition(ClassDefinition node) {
		...
		/*
		 * if (node.getFSuperType().size() > 0) { result += " -> "; result +=
		 * ppComaSeparatedNodes(node.getFSuperType()); }
		 */
		return result;
	}
	...
	public String ppTypeVariableDeclaration(EList tparams) {
		...
		while (it.hasNext()) {
			...
			if (node.getSupertype() != null) {
				result += " : "	+ TypeHelper.getName((Type) node);
			}
			if (it.hasNext())
				result += ", ";
		}
	...
	public Object visitProperty(Property node) {
		String result = node.getName() + " : "
				+ ppTypeFromMultiplicityElement(node);
		return result;
	}
	...
	public Object visitFunctionType(FunctionType node) {
		return "< " + this.accept(node.getLeft()) + "->"
	 ...
	 */
	public Object visitClass(
			fr.irisa.triskell.kermeta.language.structure.Class node) {
