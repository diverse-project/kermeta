/* $Id: KMT2KMPass4.java,v 1.4 2007-08-08 12:57:15 dvojtise Exp $
	...
	*/
	...
	public boolean beginVisit(ClassDecl classDecl) {
		...
		if (ClassDefinitionHelper.isSuperClassOf(context.current_class, context.current_class)) {
			builder.error("PASS 4 : Cycle in the inheritance tree - The type hierachy of class '"+context.current_class.getName()+"' is inconsistant.", classDecl);
			return false;
		}
...
//				builder.messages.addMessage(new KMTUnitLoadError("PASS 4 :No super operation found for method '"+builder.current_operation.getName()+"'.", operation));
				...
				if (operation.getSuperSelection() != null) {
					...
					if (!provided_name.equals(scname)) {
						builder.error("PASS 4 : Wrong super operation selection directive '"+provided_name+"', super operation selected from class '"+scname+"'.", operation);
					}
				}
			}
			// FIXED : superoperation finding is not correct
				...
				if ( (operation.getSuperSelection() == null) && possible_selection.contains("kermeta::language::structure::Object") )
					...
					if (superop != null) { 
						...
						builder.error("PASS 4 :Wrong super operation selection directive '"+provided_name+"', expecting one of "+possible_selection+".", operation.getSuperSelection());
						return false;
					}
				}
				else { // error
					builder.error("PASS 4 :Several super operation found for method '"+context.current_operation.getName()+"', : "+possible_selection+".", operation);
					return false;
				}
			}
		}
	...
	public boolean beginVisit(Property property) {
		...
		if (property.getOppositeName() != null) {
			...
			if (!(context.current_property.getType() instanceof fr.irisa.triskell.kermeta.language.structure.Class)) {
				builder.error("PASS 4 : Unexpected opposite - The type of a property that have an opposite should be a Class.", property.getOppositeName());
				return false;
			}
			...
			if (oposite_property == null) {
				builder.error("PASS 4 : Unable to resolve opposite of property " + context.current_property.getName(), property.getOppositeName());
				return false;
			}
