/* $Id: KM2EcorePass2.java,v 1.1 2007-10-17 08:56:19 cfaucher Exp $
...
public class KM2EcorePass2 extends KermetaOptimizedVisitor{
	...
	public Object visitClass(fr.irisa.triskell.kermeta.language.structure.Class node)
	{
		...
		{	// maybe this is new reference to a primitive type or a class defined in another file
			...
			String type_name = NamedElementHelper.getMangledQualifiedName(node.getTypeDefinition(), NamedElementHelper.simplePointSeparator);
	...
	{
		String type_name = NamedElementHelper.getMangledQualifiedName(node, NamedElementHelper.simplePointSeparator);
	...
	protected EOperation getEObjectForOperation(Operation node)
	{
		if (kmt2ecoremapping.containsKey(node))
			...
			return (EOperation)getEObjectForQualifiedName(
					NamedElementHelper.getMangledQualifiedName(node, NamedElementHelper.simplePointSeparator),
					NamedElementHelper.getMangledQualifiedName(node.getOwningClass(), NamedElementHelper.simplePointSeparator));
	}
	...
	protected EParameter getEObjectForParameter(Parameter node)
	{
		if (kmt2ecoremapping.containsKey(node))
			...
			return (EParameter)getEObjectForQualifiedName(
					NamedElementHelper.getMangledQualifiedName(node, NamedElementHelper.simplePointSeparator),
					NamedElementHelper.getMangledQualifiedName(node.getOperation().getOwningClass(), NamedElementHelper.simplePointSeparator));
	}
	...
	public void setSuperOperationAnnotation(Operation superOperation, EOperation newEOperation) {
		ecoreExporter.addAnnotation( 
				...
				NamedElementHelper.getMangledQualifiedName(superOperation.getOwningClass(), NamedElementHelper.simplePointSeparator),
	...
	public void setRaisedExceptionAnnotation(fr.irisa.triskell.kermeta.language.structure.Class anException, EOperation newEOperation) {
		...
		ecoreExporter.addConstraintAnnotation( 
				...
				NamedElementHelper.getMangledQualifiedName(anException.getTypeDefinition(), NamedElementHelper.simplePointSeparator), // Only decorative info, currently useless
				NamedElementHelper.getMangledQualifiedName(anException.getTypeDefinition(), NamedElementHelper.simplePointSeparator), // Idem
