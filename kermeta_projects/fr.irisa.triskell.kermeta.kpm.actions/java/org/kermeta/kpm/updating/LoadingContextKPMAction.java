...
public class LoadingContextKPMAction implements IAction {
	...
	public void execute(Out out, Unit unit, Map<String, Object> args, IProgressMonitor monitor) {
		...
		try {
			...
			// create a loading context for all these files
			//LoadersContext loadingContext = LoaderPlugin.getDefault().getNewLoadersContext(); 
			/*
			 * 
			...
			for ( Unit u : l ) {
				...
				try {
					//loadingContext.load(u.getName(), args);
			...
			//LoaderPlugin.getDefault().loadingContext.getLoaders().clear();
			/*
			 * 
