...
public class DependencyHelper {
	...
	static private void addTypecheckingDependenciesToKMTFile(File kmtFile) {
		
		KPM kpm = kmtFile.getKpm();
		
		String fileName = StringHelper.getNewNameWithExtension(kmtFile, ".km");
		
		File kmFile = kpm.findFile(fileName, kmtFile.getPath());
		if ( kmFile == null ) {
			kmFile = kpm.createFile( fileName, kmtFile.getPath() );
		...
		kpm.createDependency(kmtFile, kmFile, "typechecking", "simple_typecheck", actions);
		kpm.createDependency(kmFile, kmtFile, "typechecking", "simple_typecheck", actions);
