...
public class PrologFlora extends FloraConstants
{
    ...
    /* Function to use flLoad to load FLORA-2 file into moduleName
    ** fileName   : name of the file to load
    ** moduleName : name of FLORA module in which to load
    */
    public void loadFile(String fileName,String moduleName)
    {
	String cmd = "flLoad('"+fileName + "'>>" + moduleName+")";
	try {
	    boolean flag = engine.command(cmd);
	}
    ...
    */
    public Object[] FloraCommand(String cmd,Vector vars)
    {
        StringBuffer sb = new StringBuffer();
	String varsString = "";
	for (int i=0; i<vars.size(); i++) {
	    String floravar = (String)vars.elementAt(i);
		...
		throw new FlrException("j2flora2: Illegal variable name "
				       ...
				       + "start with a `?'");
	    if (i>0) varsString += ",";
	    varsString += "'" + (String)vars.elementAt(i) + "'=__Var" + i;
	}
