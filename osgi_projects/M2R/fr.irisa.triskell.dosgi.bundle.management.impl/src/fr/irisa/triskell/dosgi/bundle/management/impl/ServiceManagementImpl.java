...
public class ServiceManagementImpl implements ServiceManagement {
	...
	public void bindService(Object attribut, long id) {
		Object service = bundleContext.getService(serviceRegistrations.get(id)
				.getReference());
	...
	@Override
	public long moveService(long id, ServiceManagement remoteManager,
	...
	@Override
	public long duplicateService(long id, ServiceManagement remoteManager,
			boolean restart, boolean remote) {
		...
		remoteManager.bindService(remoteBundleManager, remoteManager
				.getBundleManagerId());
		...
		long remoteBundleId = bundleManager.duplicateBundle(bundle
				.getBundleId(), remoteBundleManager);
		...
		remoteManager.defineFiltering(Arrays.asList((String[]) registration
				.getReference().getProperty(Constants.OBJECTCLASS)),
				remoteBundleId);

		if (remoteBundleManager.start(remoteBundleId)) {
			Long[] remoteServiceIds = remoteManager.findServiceIds(null, "("
					+ Constants.OBJECTCLASS
					+ "="
					+ registration.getReference().getProperty(
							Constants.OBJECTCLASS) + ")", remoteBundleId);

			// get the State of the Service if it is necessary
			Object service = bundleContext.getService(registration
					.getReference());

			for (long remoteServiceId : remoteServiceIds) {

				// change the service properties to become remote service
				if (remote) {
	...
	public long registerBundleManagement(Object service,
