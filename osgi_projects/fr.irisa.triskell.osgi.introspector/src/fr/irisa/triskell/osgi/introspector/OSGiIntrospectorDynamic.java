...
public class OSGiIntrospectorDynamic {
	...
	public void introspect(String XMIFilePath) {
		...
		for (org.osgi.framework.Bundle bundle : this.context.getBundles()) {
			...
			if (bundle.getBundleId() != context.getBundle().getBundleId()) {
				// if
				// (bundle.getLocation().equals(org.osgi.framework.Constants.SYSTEM_BUNDLE_LOCATION))
				// {
				try {
				 ...
				 * OSGiIntrospectorUtil.getFileLocation(bundle); validGeneration =
				 * validGeneration && this.generateBundle(filePath);
				 * 
				 * Dictionary<String, String> dictionary = bundle.getHeaders();
				 * Enumeration<String> manifestEntries = dictionary.keys();
				 * 
				 * while (manifestEntries.hasMoreElements()) { String key =
				 * manifestEntries.nextElement(); String value =
				 * dictionary.get(key); System.out.println(key + ": " + value); }
				 * 
				 * System.out.println(); }
				 */
			}
	...
	/*
	 * private boolean generateSystemBundle(org.osgi.framework.Bundle
	 * systemBundle) throws IOException { // TODO How create the System Bundle
	 * Bundle systemBundleRepresentation =
	 * SystemBundleIntrospector.getSystemBundleRepresentation(systemBundle);
	 * systemBundleRepresentation.setLocation(org.osgi.framework.Constants.SYSTEM_BUNDLE_LOCATION);
	 * systemBundleRepresentation.setSymbolicName(systemBundle.getSymbolicName());
	 * 
	 * String manifestStringRepresentation = ""; Dictionary<String, String>
	 * dictionary = systemBundle.getHeaders(); Enumeration<String>
	 * manifestEntries = dictionary.keys();
	 * 
	 * while (manifestEntries.hasMoreElements()) { String key =
	 * manifestEntries.nextElement(); String value = dictionary.get(key);
	 * manifestStringRepresentation += key + ": " + value + "\n"; } java.io.File
	 * manifestFile; manifestFile = java.io.File.createTempFile("manifest",
	 * ".mf"); manifestFile.deleteOnExit(); BufferedWriter writer = new
	 * BufferedWriter(new OutputStreamWriter( new
	 * FileOutputStream(manifestFile)));
	 * writer.write(manifestStringRepresentation); writer.flush();
	 * writer.close();
	 * 
	 * MANIFEST manifestRepresentation = manifestFactory.createMANIFEST(); try {
	 * this.parse(manifestRepresentation, manifestFile,
	 * systemBundleRepresentation); } catch (ParserException e) {
	 * System.err.println(systemBundleRepresentation.getLocation());
	 * e.printStackTrace(); System.err.println("There is an unknown error when
	 * we try to parser the MANIFEST file on the bundle " +
	 * systemBundleRepresentation.getLocation() + "."); return false; } catch
	 * (LexerException e) {
	 * System.err.println(systemBundleRepresentation.getLocation());
	 * e.printStackTrace(); System.err.println("There is an unknown error when
	 * we try to parser the MANIFEST file on the bundle " +
	 * systemBundleRepresentation.getLocation() + "."); // e.printStackTrace();
	 * return false; } catch (IOException e) {
	 * System.err.println(systemBundleRepresentation.getLocation());
	 * e.printStackTrace(); System.err.println("There is an unknown error when
	 * we try to parser the MANIFEST file on the bundle " +
	 * systemBundleRepresentation.getLocation() + "."); return false; }
	 * systemBundleRepresentation.setManifest(manifestRepresentation);
	 * 
	 * 
	 * this.framework.addBundle(systemBundleRepresentation); return true; }
	 */

	private boolean generateBundle(org.osgi.framework.Bundle bundle)
			throws IOException {
		// TODO How create the System Bundle
		Bundle bundleRepresentation = BundleIntrospector
				.getSystemBundleRepresentation(bundle);
		...
		bundleRepresentation.setManifest(manifestRepresentation);
	...
	/*
	 * private boolean generateBundle(String bundlePath) { java.io.File f = new
	 * java.io.File(bundlePath); if (!f.exists()) { return false; } if
	 * (f.isDirectory()) { return this.generateWithDirectory(f); } else { try {
	 * return this.generateWithJar(f); } catch (IOException e) {
	 ...
	 * String[] bundlePaths = directory.list(); for (String path : bundlePaths) {
	 * java.io.File tmp = new java.io.File(directory.getAbsolutePath() +
	 * java.io.File.separator + path); SystemEntry f; if (tmp.isDirectory()) { f =
	 * jarFactory.createFolder(); f.setFullPath(folder.getFullPath() +
	 * tmp.getName() + "/"); f.setName(tmp.getName());
	 * this.generateSystemEntries(tmp, (Folder) f); folder.addEntry(f); } else
	 * if (tmp.isFile()) { f = jarFactory.createFile();
	 * f.setFullPath(folder.getFullPath() + tmp.getName());
	 * f.setName(tmp.getName()); folder.addEntry(f); } } }
	 */
	 ...
	 * java.io.File manifestFileTmp = new
	 * java.io.File(directory.getAbsolutePath() + java.io.File.separator +
	 * "META-INF" + java.io.File.separator + "MANIFEST.MF"); if
	 * (manifestFileTmp.isFile()) { Bundle bundle =
	 * frameworkFactory.createBundle();
	 * bundle.setLocation(directory.getAbsolutePath());
	 * 
	 * Folder rootFolder = jarFactory.createFolder(); rootFolder.setName("");
	 * rootFolder.setFullPath(""); bundle.setFolder(rootFolder);
	 * 
	 * Package defaultPackage = jarFactory.createPackage();
	 * defaultPackage.setName(""); defaultPackage.setFullPath("");
	 * bundle.setPackage(defaultPackage);
	 * 
	 * this.generateSystemEntries(directory, rootFolder);
	 * 
	 * try { java.io.File manifestFile =
	 * OSGiIntrospectorUtil.removeEOLBlank(manifestFileTmp); MANIFEST
	 * manifestRepresentation = manifestFactory.createMANIFEST();
	 * this.parse(manifestRepresentation, manifestFile, bundle);
	 * bundle.setManifest(manifestRepresentation);
	 * bundle.setLocation(directory.getAbsolutePath());
	 * framework.addBundle(bundle); } catch (ParserException e) {
	 * //System.out.println(e.getMessage());
	 * //System.out.println(e.getToken().getText());
	 * //System.out.println(e.getToken().getClass());
	 * System.err.println(bundle.getLocation()); e.printStackTrace();
	 * System.err.println("This error must not appears.");
	 * System.err.println("If it appears, maybe there are errors into the
	 * grammar."); return false; } catch (LexerException e) {
	 * //System.out.println(e.getMessage());
	 * System.err.println(bundle.getLocation()); e.printStackTrace();
	 * System.err.println("This error must not appears.");
	 * System.err.println("If it appears, maybe there are errors into the
	 * grammar."); // e.printStackTrace(); return false; } catch (IOException e) {
	 * System.err.println(bundle.getLocation()); e.printStackTrace();
	 * System.err.println("There is an unknown error when we try to parser the
	 * MANIFEST file on the bundle " + bundle.getLocation() + "."); return
	 * false; } } return this.translation.isValidTranslation(); }
	 */
	 ...
	 * JarFile jar = new JarFile(jarFile); Manifest manifest =
	 * jar.getManifest(); if (manifest != null) {
	 * 
	 ...
	 *  // This test is used because the Manifest function write(OutputStream)
	 * can return a valid value eexcept if Manifest-Version entry is not define
	 * into the Manifest file. if
	 * (manifest.getMainAttributes().get(Attributes.Name.MANIFEST_VERSION) ==
	 * null) {
	 * manifest.getMainAttributes().put(Attributes.Name.MANIFEST_VERSION,
	 * "1.0"); manifest.write(System.out); }
	 * 
	 * java.io.File manifestFileTmp = java.io.File.createTempFile("manifesttmp",
	 * ".mf"); manifestFileTmp.deleteOnExit(); manifest.write(new
	 * FileOutputStream(manifestFileTmp)); java.io.File manifestFile =
	 * OSGiIntrospectorUtil.removeEOLBlank(manifestFileTmp);
	 * manifestFile.deleteOnExit();
	 * 
	 * MANIFEST manifestRepresentation = manifestFactory.createMANIFEST(); try {
	 * this.parse(manifestRepresentation, manifestFile, bundle); } catch
	 * (ParserException e) { System.err.println(bundle.getLocation());
	 * e.printStackTrace(); System.err.println("There is an unknown error when
	 * we try to parser the MANIFEST file on the bundle " + bundle.getLocation() +
	 * "."); return false; } catch (LexerException e) {
	 * System.err.println(bundle.getLocation()); e.printStackTrace();
	 * System.err.println("There is an unknown error when we try to parser the
	 * MANIFEST file on the bundle " + bundle.getLocation() + "."); //
	 * e.printStackTrace(); return false; } catch (IOException e) {
	 * System.err.println(bundle.getLocation()); e.printStackTrace();
	 * System.err.println("There is an unknown error when we try to parser the
	 * MANIFEST file on the bundle " + bundle.getLocation() + "."); return
	 * false; } bundle.setManifest(manifestRepresentation); return
	 * framework.addBundle(bundle); } return
	 * this.translation.isValidTranslation(); }
	 */
