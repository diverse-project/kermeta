 ...
 * 
 */
public class BlockPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public BlockPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject block, String editing_mode) {
		...
		partKey = BehaviorViewsRepository.Block.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final BlockPropertiesEditionPart basePart = (BlockPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(BehaviorViewsRepository.Block.Properties.kTag)) {
				kTagSettings = new ReferencesTableSettings(block, StructurePackage.eINSTANCE.getKermetaModelElement_KTag());
				basePart.initKTag(kTagSettings);
			}
			if (isAccessible(BehaviorViewsRepository.Block.Properties.kType)) {
				// init part
				kTypeSettings = new EObjectFlatComboSettings(block, StructurePackage.eINSTANCE.getKermetaModelElement_KType());
				basePart.initKType(kTypeSettings);
				// set the button mode
				basePart.setKTypeButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(BehaviorViewsRepository.Block.Properties.staticType)) {
				// init part
				staticTypeSettings = new EObjectFlatComboSettings(block, BehaviorPackage.eINSTANCE.getExpression_StaticType());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == BehaviorViewsRepository.Block.Properties.kTag) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KTag();
		}
		if (editorKey == BehaviorViewsRepository.Block.Properties.kType) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KType();
		}
		if (editorKey == BehaviorViewsRepository.Block.Properties.staticType) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (BehaviorViewsRepository.Block.Properties.kTag == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Block.Properties.kType == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Block.Properties.staticType == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			BlockPropertiesEditionPart basePart = (BlockPropertiesEditionPart)editingPart;
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KTag().equals(msg.getFeature())  && isAccessible(BehaviorViewsRepository.Block.Properties.kTag))
				basePart.updateKTag();
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Block.Properties.kType))
				basePart.setKType((EObject)msg.getNewValue());
			if (BehaviorPackage.eINSTANCE.getExpression_StaticType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Block.Properties.staticType))
	...
	public boolean isRequired(Object key, int kind) {
		return key == BehaviorViewsRepository.Block.Properties.kType;
	}
