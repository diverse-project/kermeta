 ...
 * 
 */
public class VoidTypePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public VoidTypePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject voidType, String editing_mode) {
		super(editingContext, voidType, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = StructureViewsRepository.class;
		partKey = StructureViewsRepository.VoidType.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final VoidType voidType = (VoidType)elt;
			final VoidTypePropertiesEditionPart basePart = (VoidTypePropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(StructureViewsRepository.VoidType.Properties.kTag)) {
				kTagSettings = new ReferencesTableSettings(voidType, StructurePackage.eINSTANCE.getKermetaModelElement_KTag());
				basePart.initKTag(kTagSettings);
			}
			if (isAccessible(StructureViewsRepository.VoidType.Properties.kType)) {
				// init part
				kTypeSettings = new EObjectFlatComboSettings(voidType, StructurePackage.eINSTANCE.getKermetaModelElement_KType());
				basePart.initKType(kTypeSettings);
				// set the button mode
				basePart.setKTypeButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(StructureViewsRepository.VoidType.Properties.typeContainer)) {
				// init part
				typeContainerSettings = new EObjectFlatComboSettings(voidType, StructurePackage.eINSTANCE.getType_TypeContainer());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == StructureViewsRepository.VoidType.Properties.kTag) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KTag();
		}
		if (editorKey == StructureViewsRepository.VoidType.Properties.kType) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KType();
		}
		if (editorKey == StructureViewsRepository.VoidType.Properties.typeContainer) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		VoidType voidType = (VoidType)semanticObject;
		if (StructureViewsRepository.VoidType.Properties.kTag == event.getAffectedEditor()) {
		...
		if (StructureViewsRepository.VoidType.Properties.kType == event.getAffectedEditor()) {
		...
		if (StructureViewsRepository.VoidType.Properties.typeContainer == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			VoidTypePropertiesEditionPart basePart = (VoidTypePropertiesEditionPart)editingPart;
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KTag().equals(msg.getFeature())  && isAccessible(StructureViewsRepository.VoidType.Properties.kTag))
				basePart.updateKTag();
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KType().equals(msg.getFeature()) && basePart != null && isAccessible(StructureViewsRepository.VoidType.Properties.kType))
				basePart.setKType((EObject)msg.getNewValue());
			if (StructurePackage.eINSTANCE.getType_TypeContainer().equals(msg.getFeature()) && basePart != null && isAccessible(StructureViewsRepository.VoidType.Properties.typeContainer))
	...
	public boolean isRequired(Object key, int kind) {
		return key == StructureViewsRepository.VoidType.Properties.kType;
	}
