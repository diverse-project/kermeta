...
public class KpDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	...
	private static class ReferenceCache implements org.kermeta.kp.editor.IKpReferenceCache, org.eclipse.emf.common.notify.Adapter {
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, org.kermeta.kp.editor.IKpReferenceResolveResult<ReferenceType> result) {
		try {
			org.eclipse.emf.ecore.EObject root = container;
			if (!enableScoping) {
				root = org.kermeta.kp.editor.util.KpEObjectUtil.findRootContainer(container);
			}
			while (root != null) {
	...
	private java.util.Set<org.eclipse.emf.ecore.EObject> findReferencedExternalObjects(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = org.kermeta.kp.editor.util.KpEObjectUtil.findRootContainer(object);
	 ...
	 */
	protected boolean tryToResolveIdentifierInObjectTree(String identifier, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject root, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, org.kermeta.kp.editor.IKpReferenceResolveResult<ReferenceType> result, boolean checkRootFirst) {
	...
	private boolean tryToResolveIdentifierAsURI(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, org.kermeta.kp.editor.IKpReferenceResolveResult<ReferenceType> result) {
		org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EReference reference, int position, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, org.kermeta.kp.editor.IKpReferenceResolveResult<ReferenceType> result) {
		...
		if (exactMatch == null) {
			...
			if (similarMatch != null) {
				...
				result.addQuickFix(new org.kermeta.kp.editor.mopp.KpChangeReferenceQuickFix("Replace with " + similarMatch, "IMG_TOOL_FORWARD", container, reference, oldTarget, element));
			}
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, org.kermeta.kp.editor.IKpTextResource resource) {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					String result = (String) org.kermeta.kp.editor.util.KpEObjectUtil.invokeOperation(element, operation);
					names.add(result);
				}
			}
		}
	...
	protected org.kermeta.kp.editor.IKpReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = org.kermeta.kp.editor.util.KpEObjectUtil.findRootContainer(object);
	...
	private boolean isSimilar(String identifier, Object attributeValue) {
		if (attributeValue != null && attributeValue instanceof String) {
			String name = (String) attributeValue;
			if (org.kermeta.kp.editor.util.KpStringUtil.computeLevenshteinDistance(identifier, name) <= MAX_DISTANCE) {
				return true;
			}
		}
		return false;
