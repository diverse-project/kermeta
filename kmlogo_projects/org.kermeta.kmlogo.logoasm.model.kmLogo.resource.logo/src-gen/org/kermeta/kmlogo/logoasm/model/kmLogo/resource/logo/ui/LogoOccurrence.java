/*$Id : LogoOccurrence.java v 1.3 May 8, 2010 9:18:15 AM hrambelo Exp $
* Project : org.kermeta.kmlogo.logoasm.model.kmLogo.resource.logo
* File : 	LogoOccurrence.java
* License : EPL
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.kermeta.kmlogo.logoasm.model.kmLogo.resource.logo.ui.LogoPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.kermeta.kmlogo.logoasm.model.kmLogo.resource.logo.ui.LogoPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
