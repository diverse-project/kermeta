...
public abstract class GenerationPhase {
	...
	protected EPackage getSubPackage(EPackage ePackage, String name) {
		for (Iterator ip = ePackage.getESubpackages().iterator(); ip.hasNext();) {
			EPackage subPackage = (EPackage) ip.next();
	...
	protected EOperation getOperation(EClass eClass, String name) {
		for (Iterator iop = eClass.getEOperations().iterator(); iop.hasNext();) {
			EOperation eOp = (EOperation) iop.next();
	...
	protected EParameter getParam(EOperation eOp, String name) {
		for (Iterator ip = eOp.getEParameters().iterator(); ip.hasNext();) {
			EParameter param = (EParameter) ip.next();
