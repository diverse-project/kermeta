...
public class ArtextHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	public ArtextHighlighting(art.resource.artext.IArtextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, art.resource.artext.ui.ArtextColorManager colorManager, art.resource.artext.ui.ArtextEditor editor) {
		...
		preferenceStore = art.resource.artext.mopp.ArtextPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new art.resource.artext.ui.ArtextTokenScanner(colorManager);
		occurrence = new art.resource.artext.ui.ArtextOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new art.resource.artext.ui.ArtextBracketSet(sourceviewer, "artext");
		...
		isHighlightOccurrences = preferenceStore.getBoolean(art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_OCCURRENCE_CHECKBOX);
		definitionColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_DEFINITION_COLOR));
		proxyColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.artext.ui.ArtextPreferenceConstants.EDITOR_PROXY_COLOR));
	...
	private void setHighlighting() {
		...
		if (occurrence.isPositionsChanged()) {
			...
			art.resource.artext.ui.ArtextPositionCategory.DEFINTION.toString());
			setCategoryHighlighting(document,
			art.resource.artext.ui.ArtextPositionCategory.PROXY.toString());
		}
		setCategoryHighlighting(document, art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString());
	}
	...
	private void setCategoryHighlighting(org.eclipse.jface.text.IDocument document, String category) {
		...
		if (category.equals(art.resource.artext.ui.ArtextPositionCategory.PROXY.toString())) {
		...
		for (org.eclipse.jface.text.Position position : positions) {
			...
			if (tmpPosition != null) {
				if (category.equals(art.resource.artext.ui.ArtextPositionCategory.DEFINTION.toString())) {
	...
	private void removeHighlighting() {
		...
		if (occurrence.isToRemoveHighlighting()) {
			removeHighlightingCategory(document, art.resource.artext.ui.ArtextPositionCategory.DEFINTION.toString());
			removeHighlightingCategory(document, art.resource.artext.ui.ArtextPositionCategory.PROXY.toString());
			lastStyleRange = null;
		}
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, category);
		boolean isOccurrence = (category.equals(art.resource.artext.ui.ArtextPositionCategory.DEFINTION.toString()) || category.equals(art.resource.artext.ui.ArtextPositionCategory.PROXY.toString())) && lastStyleRange != null;
		if (category.equals(art.resource.artext.ui.ArtextPositionCategory.BRACKET.toString())) {
