...
public class SmARTextMinimalModelHelper {
	...
	private final static smartadapters4ART.resource.smARText.util.SmARTextEClassUtil eClassUtil = new smartadapters4ART.resource.smARText.util.SmARTextEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + smartadapters4ART.resource.smARText.util.SmARTextStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = smartadapters4ART.resource.smARText.util.SmARTextListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if(attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + smartadapters4ART.resource.smARText.util.SmARTextStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = smartadapters4ART.resource.smARText.util.SmARTextListUtil.castListUnchecked(value);
