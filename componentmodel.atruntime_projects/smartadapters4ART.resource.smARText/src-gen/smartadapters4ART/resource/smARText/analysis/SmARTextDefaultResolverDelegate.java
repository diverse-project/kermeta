...
public class SmARTextDefaultResolverDelegate<ContainerType extends org.eclipse.emf.ecore.EObject, ReferenceType extends org.eclipse.emf.ecore.EObject> {
	
	private static class ReferenceCache implements smartadapters4ART.resource.smARText.ISmARTextReferenceCache, org.eclipse.emf.common.notify.Adapter {
	 ...
	 */
	protected void resolve(String identifier, ContainerType container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, smartadapters4ART.resource.smARText.ISmARTextReferenceResolveResult<ReferenceType> result) {
		try {
			org.eclipse.emf.ecore.EClass type = reference.getEReferenceType();
			org.eclipse.emf.ecore.EObject root = smartadapters4ART.resource.smARText.util.SmARTextEObjectUtil.findRootContainer(container);
	...
	private boolean checkElement(org.eclipse.emf.ecore.EObject element, org.eclipse.emf.ecore.EClass type, String identifier, boolean resolveFuzzy, boolean checkStringWise, smartadapters4ART.resource.smARText.ISmARTextReferenceResolveResult<ReferenceType> result) {
		if (element.eIsProxy()) {
			return true;
		}
	...
	protected String produceDeResolveErrorMessage(org.eclipse.emf.ecore.EObject refObject, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, smartadapters4ART.resource.smARText.ISmARTextTextResource resource) {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation o : element.eClass().getEAllOperations()) {
				if (o.getName().toLowerCase().endsWith(NAME_FEATURE) && o.getEParameters().size() == 0 ) {
					String result = (String) smartadapters4ART.resource.smARText.util.SmARTextEObjectUtil.invokeOperation(element, o);
	...
	private String getName(ReferenceType element) {
		...
		if(element.eIsProxy()) {
			...
			if (fragment != null && fragment.startsWith(smartadapters4ART.resource.smARText.ISmARTextContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX)) {
				fragment = fragment.substring(smartadapters4ART.resource.smARText.ISmARTextContextDependentURIFragment.INTERNAL_URI_FRAGMENT_PREFIX.length());
	...
	protected smartadapters4ART.resource.smARText.ISmARTextReferenceCache getCache(org.eclipse.emf.ecore.EObject object) {
		org.eclipse.emf.ecore.EObject root = smartadapters4ART.resource.smARText.util.SmARTextEObjectUtil.findRootContainer(object);
