 ...
 */
public class SmARTextOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "smartadapters4ART.resource.smARText.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "smartadapters4ART.resource.smARText.ui.occurences.declaration";
	...
	private smartadapters4ART.resource.smARText.ui.SmARTextTokenScanner tokenScanner;
	...
	private smartadapters4ART.resource.smARText.ISmARTextTextResource textResource;
	 ...
	 */
	public SmARTextOccurrence(smartadapters4ART.resource.smARText.ISmARTextTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, smartadapters4ART.resource.smARText.ui.SmARTextTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new smartadapters4ART.resource.smARText.mopp.SmARTextMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(smartadapters4ART.resource.smARText.ui.SmARTextBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, smartadapters4ART.resource.smARText.ui.SmARTextPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, smartadapters4ART.resource.smARText.ui.SmARTextPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, smartadapters4ART.resource.smARText.ui.SmARTextPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == smartadapters4ART.resource.smARText.ui.SmARTextPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(smartadapters4ART.resource.smARText.ui.SmARTextOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(smartadapters4ART.resource.smARText.ui.SmARTextOccurrence.DECLARATION_ANNOTATION_ID);
	}
