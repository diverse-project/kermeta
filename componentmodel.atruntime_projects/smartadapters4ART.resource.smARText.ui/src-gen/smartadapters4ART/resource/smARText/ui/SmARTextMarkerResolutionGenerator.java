...
public class SmARTextMarkerResolutionGenerator implements org.eclipse.ui.IMarkerResolutionGenerator {
	...
	public org.eclipse.ui.IMarkerResolution[] getResolutions(org.eclipse.core.resources.IMarker marker) {
		try {
			...
			if (resource instanceof org.eclipse.core.resources.IFile) {
				...
				rs.getLoadOptions().put(smartadapters4ART.resource.smARText.ISmARTextOptions.DISABLE_CREATING_MARKERS_FOR_PROBLEMS, "true");
				org.eclipse.emf.ecore.resource.Resource emfResource = rs.getResource(uri, true);
				if (emfResource instanceof smartadapters4ART.resource.smARText.mopp.SmARTextResource) {
					smartadapters4ART.resource.smARText.mopp.SmARTextResource customResource = (smartadapters4ART.resource.smARText.mopp.SmARTextResource) emfResource;
					org.eclipse.emf.ecore.util.EcoreUtil.resolveAll(customResource);
					java.util.Collection<smartadapters4ART.resource.smARText.ISmARTextQuickFix> quickFixes = getQuickFixes(customResource, marker);
					java.util.List<org.eclipse.ui.IMarkerResolution2> resolutions = new java.util.ArrayList<org.eclipse.ui.IMarkerResolution2>();
					for (final smartadapters4ART.resource.smARText.ISmARTextQuickFix quickFix : quickFixes) {
						resolutions.add(new org.eclipse.ui.IMarkerResolution2() {
							...
							public void run(org.eclipse.core.resources.IMarker marker) {
								...
								try {
									...
									smartadapters4ART.resource.smARText.ui.SmARTextUIPlugin.logError("Exception while applying quick fix", e);
								}
							}
			...
			smartadapters4ART.resource.smARText.ui.SmARTextUIPlugin.logError("Exception while computing quick fix resolutions", e);
		}
		return new org.eclipse.ui.IMarkerResolution[] {};
	}
	
	public java.util.Collection<smartadapters4ART.resource.smARText.ISmARTextQuickFix> getQuickFixes(smartadapters4ART.resource.smARText.ISmARTextTextResource resource, org.eclipse.core.resources.IMarker marker) {
		...
		try {
			...
			if (quickFixContexts != null) {
				...
				for (String quickFixContext : quickFixContextParts) {
					smartadapters4ART.resource.smARText.ISmARTextQuickFix quickFix = resource.getQuickFix(quickFixContext);
					if (quickFix != null) {
