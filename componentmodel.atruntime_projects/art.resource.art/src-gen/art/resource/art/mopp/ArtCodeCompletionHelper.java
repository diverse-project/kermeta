...
public class ArtCodeCompletionHelper {
	...
	// @param cursorOffset
	// @return
	public art.resource.art.mopp.ArtCompletionProposal[] computeCompletionProposals(art.resource.art.IArtTextResource originalResource, String content, int cursorOffset) {
		...
		art.resource.art.IArtTextResource resource = (art.resource.art.IArtTextResource) resourceSet.createResource(originalResource.getURI());
		java.io.ByteArrayInputStream inputStream = new java.io.ByteArrayInputStream(content.getBytes());
		art.resource.art.mopp.ArtMetaInformation metaInformation = resource.getMetaInformation();
		art.resource.art.IArtTextParser parser = metaInformation.createParser(inputStream, null);
		art.resource.art.mopp.ArtExpectedTerminal[] expectedElements = parseToExpectedElements(parser, resource);
		if (expectedElements == null) {
			return new art.resource.art.mopp.ArtCompletionProposal[0];
		}
		if (expectedElements.length == 0) {
			return new art.resource.art.mopp.ArtCompletionProposal[0];
		}
		java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedAfterCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset));
		java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedBeforeCursor = java.util.Arrays.asList(getElementsExpectedAt(expectedElements, cursorOffset - 1));
		...
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> allProposals = new java.util.LinkedHashSet<art.resource.art.mopp.ArtCompletionProposal>();
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> rightProposals = deriveProposals(expectedAfterCursor, content, resource, cursorOffset);
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> leftProposals = deriveProposals(expectedBeforeCursor, content, resource, cursorOffset);
		...
		final java.util.List<art.resource.art.mopp.ArtCompletionProposal> sortedProposals = new java.util.ArrayList<art.resource.art.mopp.ArtCompletionProposal>(allProposals);
		java.util.Collections.sort(sortedProposals);
		return sortedProposals.toArray(new art.resource.art.mopp.ArtCompletionProposal[sortedProposals.size()]);
	}
	
	public art.resource.art.mopp.ArtExpectedTerminal[] parseToExpectedElements(art.resource.art.IArtTextParser parser, art.resource.art.IArtTextResource resource) {
		final java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements = parser.parseToExpectedElements(null, resource);
		if (expectedElements == null) {
			return new art.resource.art.mopp.ArtExpectedTerminal[0];
		}
		removeDuplicateEntries(expectedElements);
		removeInvalidEntriesAtEnd(expectedElements);
		return expectedElements.toArray(new art.resource.art.mopp.ArtExpectedTerminal[expectedElements.size()]);
	}
	
	private void removeDuplicateEntries(java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1; i++) {
			art.resource.art.mopp.ArtExpectedTerminal elementAtIndex = expectedElements.get(i);
			for (int j = i + 1; j < expectedElements.size();) {
				art.resource.art.mopp.ArtExpectedTerminal elementAtNext = expectedElements.get(j);
	...
	private void removeInvalidEntriesAtEnd(java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements) {
		for (int i = 0; i < expectedElements.size() - 1;) {
			art.resource.art.mopp.ArtExpectedTerminal elementAtIndex = expectedElements.get(i);
			art.resource.art.mopp.ArtExpectedTerminal elementAtNext = expectedElements.get(i + 1);
	...
	private String findPrefix(java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements, art.resource.art.mopp.ArtExpectedTerminal expectedAtCursor, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return "";
		}
		int end = 0;
		for (art.resource.art.mopp.ArtExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> deriveProposals(java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements, String content, art.resource.art.IArtTextResource resource, int cursorOffset) {
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> resultSet = new java.util.HashSet<art.resource.art.mopp.ArtCompletionProposal>();
		for (art.resource.art.mopp.ArtExpectedTerminal expectedElement : expectedElements) {
	...
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> deriveProposals(art.resource.art.mopp.ArtExpectedTerminal expectedTerminal, String content, art.resource.art.IArtTextResource resource, int cursorOffset) {
		art.resource.art.mopp.ArtMetaInformation metaInformation = resource.getMetaInformation();
		art.resource.art.IArtLocationMap locationMap = resource.getLocationMap();
		art.resource.art.IArtExpectedElement expectedElement = (art.resource.art.IArtExpectedElement) expectedTerminal.getTerminal();
	...
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> deriveProposals(art.resource.art.mopp.ArtExpectedTerminal expectedElement, org.eclipse.emf.ecore.EEnum enumType, String content, int cursorOffset) {
		java.util.Collection<org.eclipse.emf.ecore.EEnumLiteral> enumLiterals = enumType.getELiterals();
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> result = new java.util.HashSet<art.resource.art.mopp.ArtCompletionProposal>();
		for (org.eclipse.emf.ecore.EEnumLiteral literal : enumLiterals) {
			...
			if (matches(proposal, prefix)) {
				result.add(new art.resource.art.mopp.ArtCompletionProposal(proposal, prefix, !"".equals(prefix), true));
			}
		}
		return result;
	}
	
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> handleNCReference(art.resource.art.mopp.ArtMetaInformation metaInformation, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EReference reference, java.lang.String prefix) {
		...
		if (mappings != null) {
			...
			for (art.resource.art.IArtReferenceMapping<org.eclipse.emf.ecore.EObject> mapping : mappings) {
				...
				resultSet.add(new art.resource.art.mopp.ArtCompletionProposal(identifier, prefix, true, true));
			}
			return resultSet;
		}
		return java.util.Collections.emptyList();
	...
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> handleAttribute(art.resource.art.mopp.ArtMetaInformation metaInformation, art.resource.art.mopp.ArtExpectedStructuralFeature expectedFeature, org.eclipse.emf.ecore.EObject container, org.eclipse.emf.ecore.EAttribute attribute, java.lang.String prefix) {
			...
			if (tokenName != null) {
				...
				if (tokenResolver != null) {
					...
					if (matches(defaultValueAsString, prefix)) {
						resultSet.add(new art.resource.art.mopp.ArtCompletionProposal(defaultValueAsString, prefix, !"".equals(prefix), true));
					}
					return resultSet;
				}
			}
	...
	private java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> deriveProposal(art.resource.art.mopp.ArtExpectedCsString csString, String content, String prefix, int cursorOffset) {
		String proposal = csString.getValue();
		java.util.Collection<art.resource.art.mopp.ArtCompletionProposal> result = new java.util.HashSet<art.resource.art.mopp.ArtCompletionProposal>();
		if (matches(proposal, prefix)) {
			result.add(new art.resource.art.mopp.ArtCompletionProposal(proposal, prefix, !"".equals(prefix), false));
		}
		return result;
	}
	...
	private void setPrefixes(java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedElements, String content, int cursorOffset) {
		if (cursorOffset < 0) {
			return;
		}
		for (art.resource.art.mopp.ArtExpectedTerminal expectedElement : expectedElements) {
	...
	public art.resource.art.mopp.ArtExpectedTerminal[] getElementsExpectedAt(art.resource.art.mopp.ArtExpectedTerminal[] allExpectedElements, int cursorOffset) {
		java.util.List<art.resource.art.mopp.ArtExpectedTerminal> expectedAtCursor = new java.util.ArrayList<art.resource.art.mopp.ArtExpectedTerminal>();
		for (int i = 0; i < allExpectedElements.length; i++) {
			art.resource.art.mopp.ArtExpectedTerminal expectedElement = allExpectedElements[i];
		...
		return expectedAtCursor.toArray(new art.resource.art.mopp.ArtExpectedTerminal[expectedAtCursor.size()]);
	}
	
	private int getEnd(art.resource.art.mopp.ArtExpectedTerminal[] allExpectedElements, int indexInList) {
		art.resource.art.mopp.ArtExpectedTerminal elementAtIndex = allExpectedElements[indexInList];
		...
		for (int i = indexInList + 1; i < allExpectedElements.length; i++) {
			art.resource.art.mopp.ArtExpectedTerminal elementAtI = allExpectedElements[i];
	...
	private boolean matches(java.lang.String proposal, java.lang.String prefix) {
		return (proposal.startsWith(prefix) || art.resource.art.util.ArtStringUtil.matchCamelCase(prefix, proposal) != null) && !proposal.equals(prefix);
	}
