...
public class ArtHighlighting implements org.eclipse.jface.viewers.ISelectionProvider, org.eclipse.jface.viewers.ISelectionChangedListener {
	...
	private final static art.resource.art.ui.ArtPositionHelper positionHelper = new art.resource.art.ui.ArtPositionHelper();
	private boolean isHighlightBrackets = true;
	private boolean isHighlightOccurrences = true;
	private art.resource.art.ui.ArtTokenScanner scanner;
	...
	private art.resource.art.ui.ArtOccurrence occurrence;
	private art.resource.art.ui.ArtBracketSet bracketSet;
	...
	private final class UpdateHighlightingListener implements org.eclipse.swt.events.KeyListener, org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.MouseListener, art.resource.art.IArtBackgroundParsingListener {
	...
	public ArtHighlighting(art.resource.art.IArtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceviewer, art.resource.art.ui.ArtColorManager colorManager, art.resource.art.ui.ArtEditor editor) {
		this.display = org.eclipse.swt.widgets.Display.getCurrent();
		sourceviewer.getSelectionProvider();
		preferenceStore = art.resource.art.mopp.ArtPlugin.getDefault().getPreferenceStore();
		textWidget = sourceviewer.getTextWidget();
		projectionViewer = sourceviewer;
		scanner = new art.resource.art.ui.ArtTokenScanner(colorManager);
		occurrence = new art.resource.art.ui.ArtOccurrence(textResource, sourceviewer, scanner);
		bracketSet = new art.resource.art.ui.ArtBracketSet(sourceviewer, "art");
		this.colorManager = colorManager;
		isHighlightBrackets = preferenceStore.getBoolean(art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX);
		isHighlightOccurrences = preferenceStore.getBoolean(art.resource.art.ui.ArtPreferenceConstants.EDITOR_OCCURRENCE_CHECKBOX);
		definitionColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.art.ui.ArtPreferenceConstants.EDITOR_DEFINITION_COLOR));
		proxyColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.art.ui.ArtPreferenceConstants.EDITOR_PROXY_COLOR));
		bracketColor = colorManager.getColor(org.eclipse.jface.preference.PreferenceConverter.getColor(preferenceStore, art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR));
		black = colorManager.getColor(new org.eclipse.swt.graphics.RGB(0, 0, 0));
		
		addListeners(editor);
	}
	
	private void addListeners(art.resource.art.ui.ArtEditor editor) {
	...
	private void setHighlighting() {
		...
		if (occurrence.isPositionsChanged()) {
			...
			art.resource.art.ui.ArtPositionCategory.DEFINTION.toString());
			setCategoryHighlighting(document,
			art.resource.art.ui.ArtPositionCategory.PROXY.toString());
		}
		setCategoryHighlighting(document, art.resource.art.ui.ArtPositionCategory.BRACKET.toString());
	}
	...
	private void setCategoryHighlighting(org.eclipse.jface.text.IDocument document, String category) {
		...
		for (org.eclipse.jface.text.Position position : positions) {
			...
			if (tmpPosition != null) {
				if (category.equals(art.resource.art.ui.ArtPositionCategory.DEFINTION.toString())) {
				...
				if (category.equals(art.resource.art.ui.ArtPositionCategory.BRACKET.toString())) {
					styleRange = getStyleRangeAtPosition(tmpPosition);
	...
	private void removeHighlighting() {
		org.eclipse.jface.text.IDocument document = projectionViewer.getDocument();
		removeHighlightingCategory(document, art.resource.art.ui.ArtPositionCategory.BRACKET.toString());
		if (occurrence.isToRemoveHighlighting()) {
			removeHighlightingCategory(document, art.resource.art.ui.ArtPositionCategory.DEFINTION.toString());
			removeHighlightingCategory(document, art.resource.art.ui.ArtPositionCategory.PROXY.toString());
			lastStyleRange = null;
		}
	}
	...
	private void removeHighlightingCategory(org.eclipse.jface.text.IDocument document, String category) {
		org.eclipse.jface.text.Position[] positions = positionHelper.getPositions(document, category);
		boolean isOccurrence = (category.equals(art.resource.art.ui.ArtPositionCategory.DEFINTION.toString()) || category.equals(art.resource.art.ui.ArtPositionCategory.PROXY.toString())) && lastStyleRange != null;
		if (category.equals(art.resource.art.ui.ArtPositionCategory.BRACKET.toString())) {
		...
		display.syncExec(new Runnable() {
			public void run() {
				...
				if (selectedEObject != null) {
					setSelection(new art.resource.art.ui.ArtEObjectSelection(selectedEObject, false));
				}
			}
		});
	}
	...
	private void handleContentOutlineSelection(org.eclipse.jface.viewers.ISelection selection) {
		if (!selection.isEmpty()) {
			...
			if (selectedElement instanceof org.eclipse.emf.ecore.EObject) {
				...
				if (resource instanceof art.resource.art.IArtTextResource) {
