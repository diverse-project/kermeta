...
public class ArtBracketSet {
	...
	private java.util.ArrayList<art.resource.art.IArtBracketPair> bracketPairs;
	...
	private class BracketPair implements art.resource.art.IArtBracketPair {
	...
	private class ClosingListener implements org.eclipse.swt.events.VerifyListener, org.eclipse.swt.events.ModifyListener, org.eclipse.swt.custom.VerifyKeyListener {
		...
		public void verifyText(org.eclipse.swt.events.VerifyEvent e) {
			...
			if (caret > 0 && caret < textWidget.getCharCount()) {
				art.resource.art.IArtBracketPair bracketPair = getBracketPair(textWidget.getTextRange(caret - 1, 1), textWidget.getTextRange(caret, 1));
	...
	public ArtBracketSet(org.eclipse.jface.text.source.ISourceViewer sourceViewer, String extension) {
		languageID = extension;
		this.bracketPairs = new java.util.ArrayList<art.resource.art.IArtBracketPair>();
	...
	public art.resource.art.IArtBracketPair getBracketPair(String opening, String closing) {
		for (art.resource.art.IArtBracketPair bracketPair : bracketPairs) {
	...
	public art.resource.art.IArtBracketPair getBracketPair(int index) {
		try {
	...
	public boolean setClosingEnabledInside(art.resource.art.IArtBracketPair bracketPair, boolean closingEnabledInside) {
	...
	public boolean resetBrackets() {
		String bracketPairs = preferenceStore.getString(languageID + art.resource.art.ui.ArtPreferenceConstants.EDITOR_BRACKETS_SUFFIX);
	...
	public art.resource.art.IArtBracketPair remove(String opening, String closing) {
		for (art.resource.art.IArtBracketPair bracketPair : bracketPairs) {
			if (bracketPair.getOpeningBracket().equals(opening) && bracketPair.getClosingBracket().equals(closing)) {
	...
	public boolean setBrackets(String bracketSet) {
		...
		bracketPairs = new java.util.ArrayList<art.resource.art.IArtBracketPair>();
	...
	public void matchingBrackets() {
		...
		if (position != -1 && position != document.getLength()) {
			positionHelper.addPosition(document, art.resource.art.ui.ArtPositionCategory.BRACKET.toString(), position, 1);
			positionHelper.addPosition(document, art.resource.art.ui.ArtPositionCategory.BRACKET.toString(), caretOffset - 1, 1);
		}
	}
