...
// </ul>
//
public class ArtBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private art.resource.art.ui.ArtBracketSet bracketsTmp;
	
	//*
	// Creates a preference page for bracket setting.
	///
	public ArtBracketPreferencePage() {
		super();
		
		art.resource.art.IArtMetaInformation metaInformation = new art.resource.art.mopp.ArtMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		...
		bracketsTmp = new art.resource.art.ui.ArtBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + art.resource.art.ui.ArtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		if (activeEditor != null) {
			if (activeEditor instanceof art.resource.art.ui.ArtEditor) {
				art.resource.art.ui.ArtEditor emfTextEditor = (art.resource.art.ui.ArtEditor) activeEditor;
				art.resource.art.IArtTextResource resource = (art.resource.art.IArtTextResource) emfTextEditor.getResource();
		...
		bracketsTmp.setBrackets(getPreferenceStore().getString(		language + art.resource.art.ui.ArtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					art.resource.art.IArtBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					art.resource.art.IArtBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(		art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(		language + art.resource.art.ui.ArtPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(art.resource.art.ui.ArtPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		
		for (String languageID : languagesCombo.getItems()) {
			getPreferenceStore().setValue(			languageID + art.resource.art.ui.ArtPreferenceConstants.EDITOR_BRACKETS_SUFFIX,
		...
		if (editor != null && editor instanceof art.resource.art.ui.ArtEditor) {
			((art.resource.art.ui.ArtEditor) editor).invalidateTextRepresentation();
		}
	}
}
