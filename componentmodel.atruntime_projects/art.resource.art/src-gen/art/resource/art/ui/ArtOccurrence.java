...
public class ArtOccurrence {
	...
	private art.resource.art.IArtTextResource textResource;
	...
	public ArtOccurrence(art.resource.art.IArtTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, art.resource.art.ui.ArtTokenScanner tokenScanner) {
		...
		String[] tokenNames = new art.resource.art.mopp.ArtMetaInformation().getTokenNames();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, art.resource.art.ui.ArtPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, art.resource.art.ui.ArtPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
