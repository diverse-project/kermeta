...
// models.
//
public class ArtMinimalModelHelper {
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + art.resource.art.util.ArtStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = art.resource.art.util.ArtListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if(attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + art.resource.art.util.ArtStringUtil.capitalize(attribute.getName());
					}
					java.lang.Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = art.resource.art.util.ArtListUtil.castListUnchecked(value);
