...
public class KpOccurrence {
	...
	private org.kermeta.kp.editor.ui.KpTokenScanner tokenScanner;
	 ...
	 */
	public KpOccurrence(org.kermeta.kp.editor.IKpTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.kermeta.kp.editor.ui.KpTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.kermeta.kp.editor.mopp.KpMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.kermeta.kp.editor.ui.KpBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.kermeta.kp.editor.ui.KpPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.kermeta.kp.editor.ui.KpPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.kermeta.kp.editor.ui.KpPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.kermeta.kp.editor.ui.KpPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.kermeta.kp.editor.ui.KpOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.kermeta.kp.editor.ui.KpOccurrence.DECLARATION_ANNOTATION_ID);
	}
