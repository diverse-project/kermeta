 ...
 * code).
 */
public class KpDebuggerListener<ResultType, ContextType> implements Runnable {
	...
	private org.kermeta.kp.editor.debug.AbstractKpDebuggable debuggable;
	/**
	 ...
	 */
	private java.util.Map<Long, org.kermeta.kp.editor.util.KpPair<String, Object>> objectMap = new java.util.LinkedHashMap<Long, org.kermeta.kp.editor.util.KpPair<String, Object>>();
	...
	public KpDebuggerListener(int requestPort) {
		super();
	...
	private void runDebugger() throws java.io.IOException {
		...
		org.kermeta.kp.editor.debug.KpDebugMessage command;
		while (!stop) {
			command = communicationHelper.receive(reader);
			if (command == null) {
				break;
			}
			if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.EXIT)) {
				debuggable.terminate();
				stop = true;
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.RESUME)) {
				debuggable.resume();
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.STEP_OVER)) {
				debuggable.stepOver();
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.STEP_INTO)) {
				debuggable.stepInto();
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.STEP_RETURN)) {
				debuggable.stepReturn();
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.ADD_LINE_BREAKPOINT)) {
			...
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.REMOVE_LINE_BREAKPOINT)) {
				String location = command.getArgument(0);
				int line = Integer.parseInt(command.getArgument(1));
				debuggable.removeLineBreakpoint(location, line);
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_STACK)) {
				final String[] stack = debuggable.getStack();
				String controlStack = org.kermeta.kp.editor.util.KpStringUtil.encode('#', stack);
				org.kermeta.kp.editor.debug.KpDebugMessage message = new org.kermeta.kp.editor.debug.KpDebugMessage(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_STACK_RESPONSE, new String[] {controlStack});
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_FRAME_VARIABLES)) {
				...
				org.kermeta.kp.editor.debug.KpDebugMessage message = new org.kermeta.kp.editor.debug.KpDebugMessage(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_FRAME_VARIABLES_RESPONSE, topVariableIDs);
				communicationHelper.sendEvent(message, output);
			} else if (command.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_VARIABLES)) {
				...
				org.kermeta.kp.editor.debug.KpDebugMessage message = new org.kermeta.kp.editor.debug.KpDebugMessage(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.GET_VARIABLES_RESPONSE, varStrings);
				communicationHelper.sendEvent(message, output);
			} else {
	...
	private String convertToString(long id, Object object) {
		...
		return org.kermeta.kp.editor.util.KpStringUtil.convertToString(properties);
	}
	...
	private long getObjectID(String name, Object value) {
		org.kermeta.kp.editor.util.KpPair<String, Object> pair = new org.kermeta.kp.editor.util.KpPair<String, Object>(name, value);
	...
	public org.kermeta.kp.editor.debug.AbstractKpDebuggable getDebuggable() {
		return debuggable;
	}
	
	public void setDebuggable(org.kermeta.kp.editor.debug.AbstractKpDebuggable debuggable) {
