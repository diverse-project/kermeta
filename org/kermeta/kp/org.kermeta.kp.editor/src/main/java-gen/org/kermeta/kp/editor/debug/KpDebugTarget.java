...
public class KpDebugTarget extends org.kermeta.kp.editor.debug.KpDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.kermeta.kp.editor.debug.IKpDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.kermeta.kp.editor.debug.KpDebugProcess process;
	...
	private org.kermeta.kp.editor.debug.KpDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.kermeta.kp.editor.debug.IKpDebugEventListener> eventListeners = new java.util.ArrayList<org.kermeta.kp.editor.debug.IKpDebugEventListener>();
	
	private org.kermeta.kp.editor.debug.KpDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.kermeta.kp.editor.mopp.KpMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.kermeta.kp.editor.debug.KpDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.kermeta.kp.editor.debug.KpDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.kermeta.kp.editor.debug.IKpDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public KpDebugTarget(org.kermeta.kp.editor.debug.KpDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.kermeta.kp.editor.debug.KpDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.kermeta.kp.editor.debug.KpDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.kermeta.kp.editor.debug.IKpDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.kermeta.kp.editor.debug.IKpDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.kermeta.kp.editor.mopp.KpMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.kermeta.kp.editor.debug.KpLineBreakpoint lineBreakpoint = (org.kermeta.kp.editor.debug.KpLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.kermeta.kp.editor.debug.KpDebugMessage message) {
		try {
			if (message.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.RESUMED)) {
	...
	public org.kermeta.kp.editor.debug.KpDebugProxy getDebugProxy() {
