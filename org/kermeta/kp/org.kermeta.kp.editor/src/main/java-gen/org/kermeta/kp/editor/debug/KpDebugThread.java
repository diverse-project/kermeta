...
public class KpDebugThread extends org.kermeta.kp.editor.debug.KpDebugElement implements org.eclipse.debug.core.model.IThread, org.kermeta.kp.editor.debug.IKpDebugEventListener {
	
	private boolean suspended = false;
	private org.kermeta.kp.editor.debug.KpDebugTarget debugTarget;
	
	public KpDebugThread(org.kermeta.kp.editor.debug.KpDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			org.kermeta.kp.editor.debug.KpDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = org.kermeta.kp.editor.util.KpStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new org.kermeta.kp.editor.debug.KpStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public org.kermeta.kp.editor.debug.KpDebugTarget getTarget() {
		return debugTarget;
	}
