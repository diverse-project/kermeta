...
public abstract class AbstractKpDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.kermeta.kp.editor.util.KpPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.kermeta.kp.editor.util.KpPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			org.kermeta.kp.editor.mopp.KpPlugin.logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			org.kermeta.kp.editor.mopp.KpPlugin.logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.kermeta.kp.editor.debug.EKpDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.kermeta.kp.editor.debug.KpDebugMessage message = new org.kermeta.kp.editor.debug.KpDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.kermeta.kp.editor.util.KpPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.kermeta.kp.editor.util.KpPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.kermeta.kp.editor.util.KpPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.kermeta.kp.editor.debug.EKpDebugMessageTypes.STARTED, true);
			setSuspend(true);
