  ...
  def visit(par : ClassDefinition){
   // try{
      if (!Util.hasCompilerIgnoreTag(par)){
        log.debug("   generating scala code for ClassDefinition "+par.getName)
       ...
       // println(par.eContainer.asInstanceOf[PackageAspect].getQualifiedNameCompilo)
        ...
        log.debug("   generation of scala code for ClassDefinition "+par.getName +" DONE")
      }
  /*  }
    catch{
      case e => log.error("failed to generate scala code for ClassDefinition "+par.getName+ " due to internal error "+ e.getMessage,e)
    }*/
  }
