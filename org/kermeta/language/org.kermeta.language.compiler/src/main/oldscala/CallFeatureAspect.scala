...
trait CallFeatureAspect extends CallExpressionAspect with LogAspect {
        ...
        if (this.getTarget!=null){
            if (this.getTarget.isInstanceOf[TypeLiteral]){
                if (this.getTarget.asInstanceOf[TypeLiteral].getTyperef().getType().isInstanceOf[ParameterizedType]){
                    ...
                    res.append(TypeEquivalence.getPackageEquivalence(ty.eContainer.asInstanceOf[Package].getQualifiedNameCompilo))
    ...
    def generatePropertyCall(res : StringBuilder){
        ...
        res.append(GlobalConfiguration.scalaPrefix+TypeEquivalence.getMethodEquivalence(TargetType.toString, this.getName))
    ...
    def generateOperationCall(res : StringBuilder){
        ...
        if (ops.size>0){
            ...
            //}else{
            res.append(Util.protectScalaKeyword(TypeEquivalence.getMethodEquivalence(TargetType.toString, this.getName)))

            //}
        }else{
            res.append(Util.protectScalaKeyword(TypeEquivalence.getMethodEquivalence(TargetType.toString, Util.getEcoreRenameOperation(this.getStaticOperation))))
    ...
    def generateInstanceOf(res:StringBuilder, o : org.kermeta.language.structure.Object)={
        res.append("[")
    ...
    /* TO MERGE */
    def generateIsInstanceOf(res:StringBuilder, o : org.kermeta.language.structure.Object)={
        if (o.isInstanceOf[TypeLiteral]){
            generateTarget(res);
            res.append(".");
        ...
        res.append("(scalaUtil.Util.clone(");generateTarget(res);res.append(","); this.getParameters.get(0).generateScalaCode(res);res.append("))"); 
            if (this.getTarget.isInstanceOf[TypeLiteral]){
               ...
               // if (this.getTarget.asInstanceOf[TypeLiteral].getTyperef().getType.isInstanceOf[Class]){
                    res.append("_root_." +TypeEquivalence.getTypeEquivalence(UTilScala.getQualifiedNameType(this.getTarget.asInstanceOf[TypeLiteral].getTyperef().getType, ".")))
                    res.append("]")
                //}else{
            ...
            override def generateScalaCode(res : StringBuilder) : Unit = {
                ...
                this.getName match {
                    ...
                    case "addAll"
                        if (this.getTarget != null && (this.getTarget.getStaticType != null && this.getTarget.getStaticType.isInstanceOf[org.kermeta.language.structure.Class]) //|| (this.getStaticProperty != null && this.getStaticProperty.getType.isInstanceOf[fr.irisa.triskell.kermeta.language.structure.Class]))
                            && (this.getTarget.getStaticType.asInstanceOf[org.kermeta.language.structure.Class].getTypeDefinition.getName.equals("OrderedSet")
                                ||this.getTarget.getStaticType.asInstanceOf[org.kermeta.language.structure.Class].getTypeDefinition.getName.equals("Set")))
