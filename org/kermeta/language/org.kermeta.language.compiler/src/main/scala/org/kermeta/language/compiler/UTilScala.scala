...
object UTilScala {
  ...
  def getQualifiedNamePackage(pack : org.kermeta.language.structure.Package,sep: String):String ={
   ...
   def getQualifiedNameClass(pack : org.kermeta.language.structure.GenericTypeDefinition, sep: String):String ={
       return TypeEquivalence.getPackageEquivalence(getQualifiedNamePackage(pack.eContainer().asInstanceOf[org.kermeta.language.structure.Package],sep)) + sep+ pack.getName();
    }
 def getQualifiedNameType(pack : org.kermeta.language.structure.Type, sep: String):String ={
     if (pack.isInstanceOf[org.kermeta.language.structure.Class]){
      ...
      return   TypeEquivalence.getPackageEquivalence(getQualifiedNamePackage(pack.eContainer().asInstanceOf[org.kermeta.language.structure.Package],sep) )+ sep+ pack.asInstanceOf[org.kermeta.language.structure.DataType].getName();
