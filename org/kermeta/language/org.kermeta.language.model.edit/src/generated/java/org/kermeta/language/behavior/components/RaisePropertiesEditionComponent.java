 ...
 * 
 */
public class RaisePropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public RaisePropertiesEditionComponent(PropertiesEditingContext editingContext, EObject raise, String editing_mode) {
		super(editingContext, raise, editing_mode);
		parts = new String[] { BASE_PART };
		repositoryKey = BehaviorViewsRepository.class;
		partKey = BehaviorViewsRepository.Raise.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final Raise raise = (Raise)elt;
			final RaisePropertiesEditionPart basePart = (RaisePropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(BehaviorViewsRepository.Raise.Properties.kTag)) {
				kTagSettings = new ReferencesTableSettings(raise, StructurePackage.eINSTANCE.getKermetaModelElement_KTag());
				basePart.initKTag(kTagSettings);
			}
			if (isAccessible(BehaviorViewsRepository.Raise.Properties.kType)) {
				// init part
				kTypeSettings = new EObjectFlatComboSettings(raise, StructurePackage.eINSTANCE.getKermetaModelElement_KType());
				basePart.initKType(kTypeSettings);
				// set the button mode
				basePart.setKTypeButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(BehaviorViewsRepository.Raise.Properties.staticType)) {
				// init part
				staticTypeSettings = new EObjectFlatComboSettings(raise, BehaviorPackage.eINSTANCE.getExpression_StaticType());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == BehaviorViewsRepository.Raise.Properties.kTag) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KTag();
		}
		if (editorKey == BehaviorViewsRepository.Raise.Properties.kType) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KType();
		}
		if (editorKey == BehaviorViewsRepository.Raise.Properties.staticType) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		Raise raise = (Raise)semanticObject;
		if (BehaviorViewsRepository.Raise.Properties.kTag == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Raise.Properties.kType == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Raise.Properties.staticType == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			RaisePropertiesEditionPart basePart = (RaisePropertiesEditionPart)editingPart;
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KTag().equals(msg.getFeature())  && isAccessible(BehaviorViewsRepository.Raise.Properties.kTag))
				basePart.updateKTag();
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Raise.Properties.kType))
				basePart.setKType((EObject)msg.getNewValue());
			if (BehaviorPackage.eINSTANCE.getExpression_StaticType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Raise.Properties.staticType))
	...
	public boolean isRequired(Object key, int kind) {
		return key == BehaviorViewsRepository.Raise.Properties.kType;
	}
