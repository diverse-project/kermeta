...
public class ModelPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public ModelPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject model, String editing_mode) {
		...
		partKey = StructureViewsRepository.Model.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(StructureViewsRepository.Model.Properties.kTag)) {
				kTagSettings = new ReferencesTableSettings(model, StructurePackage.eINSTANCE.getKermetaModelElement_KTag());
				basePart.initKTag(kTagSettings);
			}
			if (isAccessible(StructureViewsRepository.Model.Properties.kType)) {
				// init part
				kTypeSettings = new EObjectFlatComboSettings(model, StructurePackage.eINSTANCE.getKermetaModelElement_KType());
				basePart.initKType(kTypeSettings);
				// set the button mode
				basePart.setKTypeButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(StructureViewsRepository.Model.Properties.contents)) {
				contentsSettings = new ReferencesTableSettings(model, StructurePackage.eINSTANCE.getModel_Contents());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == StructureViewsRepository.Model.Properties.kTag) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KTag();
		}
		if (editorKey == StructureViewsRepository.Model.Properties.kType) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KType();
		}
		if (editorKey == StructureViewsRepository.Model.Properties.contents) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (StructureViewsRepository.Model.Properties.kTag == event.getAffectedEditor()) {
		...
		if (StructureViewsRepository.Model.Properties.kType == event.getAffectedEditor()) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			...
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KTag().equals(msg.getFeature())  && isAccessible(StructureViewsRepository.Model.Properties.kTag))
				basePart.updateKTag();
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KType().equals(msg.getFeature()) && basePart != null && isAccessible(StructureViewsRepository.Model.Properties.kType))
				basePart.setKType((EObject)msg.getNewValue());
			if (StructurePackage.eINSTANCE.getModel_Contents().equals(msg.getFeature())  && isAccessible(StructureViewsRepository.Model.Properties.contents))
	...
	public boolean isRequired(Object key, int kind) {
		return key == StructureViewsRepository.Model.Properties.kType;
	}
