 ...
 * 
 */
public class LoopPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	public LoopPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject loop, String editing_mode) {
		...
		partKey = BehaviorViewsRepository.Loop.class;
	}
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final LoopPropertiesEditionPart basePart = (LoopPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(BehaviorViewsRepository.Loop.Properties.kTag)) {
				kTagSettings = new ReferencesTableSettings(loop, StructurePackage.eINSTANCE.getKermetaModelElement_KTag());
				basePart.initKTag(kTagSettings);
			}
			if (isAccessible(BehaviorViewsRepository.Loop.Properties.kType)) {
				// init part
				kTypeSettings = new EObjectFlatComboSettings(loop, StructurePackage.eINSTANCE.getKermetaModelElement_KType());
				basePart.initKType(kTypeSettings);
				// set the button mode
				basePart.setKTypeButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(BehaviorViewsRepository.Loop.Properties.staticType)) {
				// init part
				staticTypeSettings = new EObjectFlatComboSettings(loop, BehaviorPackage.eINSTANCE.getExpression_StaticType());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == BehaviorViewsRepository.Loop.Properties.kTag) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KTag();
		}
		if (editorKey == BehaviorViewsRepository.Loop.Properties.kType) {
			return StructurePackage.eINSTANCE.getKermetaModelElement_KType();
		}
		if (editorKey == BehaviorViewsRepository.Loop.Properties.staticType) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (BehaviorViewsRepository.Loop.Properties.kTag == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Loop.Properties.kType == event.getAffectedEditor()) {
		...
		if (BehaviorViewsRepository.Loop.Properties.staticType == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		if (editingPart.isVisible()) {
			LoopPropertiesEditionPart basePart = (LoopPropertiesEditionPart)editingPart;
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KTag().equals(msg.getFeature())  && isAccessible(BehaviorViewsRepository.Loop.Properties.kTag))
				basePart.updateKTag();
			if (StructurePackage.eINSTANCE.getKermetaModelElement_KType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Loop.Properties.kType))
				basePart.setKType((EObject)msg.getNewValue());
			if (BehaviorPackage.eINSTANCE.getExpression_StaticType().equals(msg.getFeature()) && basePart != null && isAccessible(BehaviorViewsRepository.Loop.Properties.staticType))
	...
	public boolean isRequired(Object key, int kind) {
		return key == BehaviorViewsRepository.Loop.Properties.kType;
	}
