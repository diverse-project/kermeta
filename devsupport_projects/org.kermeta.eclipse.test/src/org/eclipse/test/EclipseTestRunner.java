...
import org.eclipse.test.plugin.Activator;
...
//        pm.upload(getClass().getName());
    ...
    private static JUnitResultFormatter createFormatter(String classname, File outfile) throws BuildException {
        ...
        try {
        	...
        	try {
				URL junitURL = FileLocator.resolve(Platform.getBundle("org.junit4").getResource("/"));
			} catch (IOException e) {
        	...
        	try {
        		System.out.println("default classloader failed to get "+classname+", Trying with org.eclipse.test ClassLoader");
				f = Class.forName(classname, true, Activator.getDefault().getClass().getClassLoader());
        	...
        	try {
				...
				System.out.println("Activator classloader failed to get "+classname+", Trying with dedicated URL ClassLoader");
				Set<URL> urlsV = new LinkedHashSet<URL>();
				String bundleName = "org.eclipse.test";
				// Plugin Dependencies
				...
				URL junitURL= null;
				try {
					junitURL = FileLocator.resolve(Platform.getBundle("org.junit4").getResource("/"));
					urlsV.add( junitURL );
					urlsV.add(new URL("file:C:\\eclipse3.3\\eclipse\\plugins\\org.junit4_4.3.1\\junit.jar"));
				} catch (IOException e) {
				...
				URLClassLoader cl = new URLClassLoader(urls, Activator.getDefault().getClass().getClassLoader());
				try {
		    		System.out.println("trying");
		    		
					f = Class.forName(classname, true, cl);
